...
void create_disk(string disk_name)
{
    ...
    cout<<fwrite(fmaps,filemap_size,no_of_inodes,fp)<<" File map written"<<endl;
    ...
    cout<<fwrite(inodes,inode_size,no_of_inodes,fp)<<" inode written"<<endl;
    ...
    cout<<fwrite(dblocks,db_size,no_of_blocks,fp)<<" DB blocks data written"<<endl;
...
int mount_disk(string disk_name)
{
    ...
    fseek(fp,0,SEEK_SET);
    cout<<fread(&sb,sizeof(struct superblock),1,fp)<<" Super block read"<<endl;
    ...
    cout<<fread(fmaps,filemap_size,no_of_inodes,fp)<<" File map read"<<endl;;


    
cout<<"File maps read"<<endl;
    for(int i=0;i<no_of_inodes;i++)
    {
...
cout<<"Data blocks metadat read"<<endl;
...
void write_file(int filedesc)
{
    ...
    cout<<"Last block size"<<totalwritesize<<endl;
